...
export type StaffShipmentStatusProps = {
	id?: string;
	role: {
		value: string;
...
export interface CreateStaffShipmentStatusProps extends StaffShipmentStatusProps {
	address: {
		value: Address;
...
export function useStaffShipmentStatus(staff: StaffProps): StaffShipmentStatusProps {
...
export function useCreateStaffShipmentStatus(
	staff: CreateStaffProps
): CreateStaffShipmentStatusProps {
	const [role, setRole] = useState(staff.role);
	const [name, setName] = useState(staff.name);
	const [username, setUsername] = useState(staff.username);
	const [email, setEmail] = useState(staff.email);
